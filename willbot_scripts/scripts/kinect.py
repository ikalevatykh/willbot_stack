...
class Kinect():
    def __init__(self, depth=False, rgb=False, point_cloud=False, depth_conv=0):
        ...
        if rgb:
            ...
            print('Waiting for depth data ({})'.format(rgb_topic))
            self._rgb_msg = None

            def rgb_cb(msg):
                self._rgb_msg = msg

            s = rospy.Subscriber(rgb_topic, Image, rgb_cb, queue_size=1)
            while self._rgb_msg is None:
                rospy.sleep(0.1)
        ...
        if point_cloud:
            ...
            s = rospy.Subscriber(points_topic, PointCloud2,
                                 points_cb, queue_size=1)
    ...
    def depth_uint8(self):
        ...
        if self._depth_conv_version == 0:
            ...
            im_pil = PIL_Image.fromarray(
                (im / max_depth_real * 255).astype(np.uint8))
    ...
    def rgb_image(self):
        if self._rgb_msg is None:
            return None, None
        msg = self._rgb_msg
        image = self._bridge.imgmsg_to_cv2(msg, 'rgb8')  # 'bgr8')
    ...
    def point_cloud(self):
        if self._points_msg is None:
            return None
        points, transform = transform_point_cloud(
            self._listener, self._points_msg, '/world')
        return points
