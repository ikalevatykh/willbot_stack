...
class Scalars:
    ...
    def __setitem__(self, key, value):
        if isinstance(key, tuple):
            ...
            raise ValueError('{} ({}) is not a valid key type.'.format(key, type(key)))

        clean_value = self.clean_entry(value)
    ...
    def update_scalars(self, scalars):
        ...
        for space in spaces:
            for key in sc[space].keys():
                ...
                stats[space][key] = stack[space][key].mean(axis=0),\
                                    stack[space][key].std(axis=0)+1e-8
